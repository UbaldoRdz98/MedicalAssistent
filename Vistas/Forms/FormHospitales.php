            ...
            foreach($reg as $valor)
            {
                switch($valor->TipoUsuario)
                {
                    case 'Admin':
                        ...
                        ?>
                    <div class="container">
                    <h1>Editar Hospitales</h1>
                    <form method="POST">
                        <div class="mb-3 w-100">
                        <label class='class-label'>Empresa:</label>
                        <?php
                            extract($_GET);
                            $id_empresa=$_GET['Hospital'];
                            $Query="SELECT e.*, edo.Id_Estado, p.Id_Pais, edo.Estado, c.Ciudad FROM empresas e JOIN ciudades c ON e.Id_Ciudad = c.Id_Ciudad JOIN estados edo ON c.Id_Estado = edo.Id_Estado JOIN paises p ON edo.Id_Pais = p.Id_Pais  WHERE Id_Empresa = '$id_empresa'";
                            $reg = $db->Selects($Query);
                            foreach($reg as $valor)
                            {
                                echo "
                                        <input type='text' name='id_empresa' placeholder='Escriba el Nombre Corto de la Empresa' class='form-control text-uppercase' value='$id_empresa' disabled>
                                    </div>
                                    <div class='mb-3 w-100'>
                                        <label class='class-label'>Nombre:</label>
                                        <input type='text' name='empresa' placeholder='Escriba el Nombre Completo de la Empresa' class='form-control' value='$valor->Empresa' required>
                                    </div>
                                    <div class='mb-3 w-100'>
                                ...
                                <div class='mb-3 w-100'>
                                    ...
                                    <input type='text' name='Numero_Exterior' placeholder='Escriba el Número Exterior' class='form-control w-50' value='$valor->Numero_Exterior' required>
                                </div>
                                <div class='mb-3 w-100'>
                                    <label class='class-label'>Número Interior:</label>
                                    <input type='text' name='Numero_Interior' placeholder='Escriba el Número Interior' class='form-control w-50' value='$valor->Numero_Interior' requiered>
                                </div>
                                <div class='mb-3 w-100'>
        ...
        if($_POST)
        {
            ...
            $query = "  UPDATE empresas
                        SET
                            ...
                            FUM                 = NOW()
                        WHERE Id_Empresa = '".strtoupper($id_empresa)."'";
                $db2->Insert($query);
                echo "<div class='container'><div class='alert alert-success'>Hospital Actualizado</div></div>";
                $db2->DesconectarDB();
                echo "<script> setTimeout(function() { window.location = '../Empresas/Hospitales.php'; }, 100); </script>";
            }
        ?>
