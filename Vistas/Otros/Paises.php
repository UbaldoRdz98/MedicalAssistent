...
<?php
    ...
    ?>
        ...
        if($_GET)
        {
            ...
            if($filtro == "All")
            {
                $Query = "SELECT * FROM paises LIMIT 100000000000000 ";
            }
            else
            {
                $Query = "SELECT * FROM paises LIMIT $filtro ";
            }

            $tabla = $conexion->Selects($Query);
            echo "  <table id='TablaPaises' class='table table-hover text-center'>
            ...
            {
                echo "<tr>
                        <form action='../Forms/FormPaisesEditar.php' method='GET'>                        
                            <td name='MyTable'>
                                <input type='hidden' name='Pais' value='$registro->Id_Pais'>$registro->Id_Pais</td>
                            </td>
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
            <button type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#Nuevo">Nuevo País</button>
        </div>
        <br>
    </div>
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header bg-dark text-white">
                    <h5 class="modal-title" id="NuevoLabel">Nuevo País</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" action="Paises.php" id="modal-paises">
                        <div class="mb-3 w-100">
                            <label class='class-label'>País:</label>
                            <input type="text" name="Pais" placeholder="Escriba el Nombre del País" class="form-control text-uppercase" required>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                    <button type="submit" class="btn btn-primary" form="modal-paises">Guardar</button>
                </div>
                <?php
                    if($_POST)
                    {
                        ...
                        $db2->ConectarDB();
                        extract($_POST);
                        $query = "INSERT INTO paises( Pais, FUM)
                                    VALUES( '$Pais', NOW())";
                        $db2->Insert($query);
                        echo "<div class='alert alert-success'>País Registrada</div>";
                        header("refresh:1; Paises.php");
                        $db2->DesconectarDB();
                    }
                ?>
            </div>
